 ...
 *
 * Full implementation:
 *   - Kernel linked list of monitored container entries
 *   - Mutex-protected list access (justified in README)
 *   - Periodic RSS polling via kernel timer (every 1 second)
 *   - Soft-limit: one-shot dmesg warning per entry
 *   - Hard-limit: SIGKILL + dmesg log, entry removed
 *   - ioctl REGISTER / UNREGISTER from user-space supervisor
 *   - Clean module unload: all list entries freed
 */
...
*   - The timer callback may call get_rss_bytes() → get_task_mm()     */
/*     which can sleep on some kernel paths. A spinlock must not be    */
/*     held across potentially-sleeping code.                          */
...
static void timer_callback(struct timer_list *t)
{
    ...
    /*
     * list_for_each_entry_safe lets us delete entries while iterating
     * without use-after-free: `tmp` always holds the next pointer
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        if (rss < 0) {
            printk(KERN_INFO "[container_monitor] Removing stale entry "
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        if (req.soft_limit_bytes > req.hard_limit_bytes) {
            printk(KERN_ERR "[container_monitor] soft > hard for pid=%d\n", req.pid);
            return -EINVAL;
        }
...
static void __exit monitor_exit(void)
{
     ...
     * ---------------------------------------------------------------- */
    {
        ...
        mutex_lock(&monitored_lock);
        list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
            printk(KERN_INFO "[container_monitor] Freeing container=%s pid=%d on unload\n",
                   entry->container_id, entry->pid);
            list_del(&entry->list);
            kfree(entry);
        }
